...
class Trafikverket(object):
	...
	def _generate_request_data(self, objecttype:str, includes: typing.List[str] = [], filters: typing.List[Filter] = []):
	...
	async def make_request(self, objecttype:str, includes: typing.List[str] = [], filters: typing.List[Filter] = []):
		...
		async with aiohttp.ClientSession(loop=self._loop) as session:
			with async_timeout.timeout(10):
				async with session.post(Trafikverket._api_url, data=request_data_text, headers=headers) as response:
					return etree.fromstring(await response.text()).xpath("/RESPONSE/RESULT/" + objecttype)
